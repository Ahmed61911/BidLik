 ...
 * Ported near-verbatim from webapp/src/lib/format.ts — keep in sync.
...
/** Same as formatDateTime but with milliseconds — for bid-order/timing displays. */
 ...
 * actually stored and PostgREST actually sent over the wire.
 */
 ...
 * milliseconds to 3 decimal places under a minute, then minutes/hours/days
 * once the sub-second digits stop mattering.
 */
...
/**
 * Price-vs-expected tier (seller/listing perspective):
...
/** NativeWind className for text color, matching the tokens in tailwind.config.js. */
...
/** Gradient color stops for <LinearGradient> (expo-linear-gradient) — RN has no CSS gradient classes. */
export function priceTierGradientColors(tier: PriceTier): [string, string] {
  if (tier === "below") return ["#DC2626", "#B91C1C"];
  if (tier === "near") return ["#E8A63D", "#D97706"];
  return ["#16A34A", "#15803D"];
}

/** Listing price tier — unified color for the current price, for every role. Reference: prixPlancher. */
...
/**
 * Buyer perspective price tier (inverse of seller):
 *  - "deal"  : current < 90% of expected      → green (bonne affaire)
