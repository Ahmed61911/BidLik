...
export const supabaseAdminPaiements = {
   ...
   * `userId` is the target user this proof/payment belongs to (payment-proofs
   * are stored per-user); `carId` is only recorded as metadata for "carPayment".
   */
  async uploadProof(
    file: File,
    userId: string,
    ...
    const category = kind === "refund" ? "admin-refund" : kind === "carPayment" ? "car-payment" : "admin-generic";
    const buildPath =
      kind === "refund"
        ? (ext: string) => paymentPaths.adminRefund(userId, ext)
        : kind === "carPayment"
          ? (ext: string) => paymentPaths.carPayment(userId, ext)
          : (ext: string) => paymentPaths.adminGeneric(userId, ext);
    const r = await storage.uploadFile({
      file,
      bucket: "payment-proofs",
      category,
      carId: kind === "carPayment" && carId ? carId : undefined,
      buildPath,
    });
