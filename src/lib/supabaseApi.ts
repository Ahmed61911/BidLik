...
export const supabaseApi: ApiClient = {
  ...
  async listBids(auctionId) {
    ...
    return ((data as Row[]) ?? []).map((r) =>
      mapBid({
        ...
        car_id: r.car_id,
        // is_own tells us WHICH rows are the caller's own — safe to attach
        // their own real id (already known client-side) so "userHasBid" /
        // "is this my bid" comparisons work on first load, not just after a
        // realtime event. Other bidders' real ids stay hidden (empty).
  ...
  async setAutoBid(auctionId, enabled, maxAmount) {
