-- get_my_won_car_details() returned raw expert_images unconditionally to the
-- auction winner -- never gated the way get_car_expertise() already gates
-- them (admin/vendeur/assigned-expert only). The frontend never rendered
-- this field (the won-car page's expertise photos/report come from
-- get_car_expertise() instead), so simplest fix: stop returning it here at
-- all rather than duplicate the same CASE WHEN gate in a second place.
