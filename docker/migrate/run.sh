...
# ----------------------------------------------------------------------------
# Storage buckets — not created by any migration file (verified: neither the
# cloud project's migration history nor this repo ever ran an
# `INSERT INTO storage.buckets`; the two buckets were created by hand in the
# cloud Studio UI). Seed them here idempotently so a fresh local stack matches
# the cloud project without any manual step.
# ----------------------------------------------------------------------------
echo "→ seed  storage buckets"
psql -v ON_ERROR_STOP=1 -q <<'SQL'
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'car-images', 'car-images', false, 10485760,
...
echo "✓ all migrations applied + buckets seeded"
