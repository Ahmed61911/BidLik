...
-- `authenticated`.
--
-- Found while testing the buyer caution-proof upload: it failed with
-- "new row violates row-level security policy" — storage-api masks the real
-- underlying Postgres error with that generic message. The actual error
-- (confirmed by calling the function directly as `authenticated`) is:
...
-- winning an auction), and caution proofs (buyers).
-- ============================================================================
